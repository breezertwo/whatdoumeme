...
export const CzarView = ({ roundData, onConfirmClicked, onCardClicked, requestMemeUrl }: CzarViewProps) => {
  return roundData.isCzar ? (
    <div className="main-container" style={{ justifyContent: 'space-around' }}>
      <>
        ...
        <Cards onCardClicked={onCardClicked} playerCards={roundData.memeCards} isCzar={roundData.isCzar} />
        <ButtonContainer onConfirmClicked={onConfirmClicked} />
      </>
    </div>
  ) : (
    <LoadingSpinner requestMemeUrl={requestMemeUrl} msg={`Wait until czar "${roundData.currentCzar}" selects a meme...`} />
  );
};
